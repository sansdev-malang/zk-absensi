        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <form action="{{ route('shifts.store') }}" method="POST" class="space-y-6">
                ...
                <div class="bg-white p-6 shadow-sm sm:rounded-2xl border border-gray-100">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="text" name="nama" value="{{ old('nama') }}" required class="w-full border-gray-300 rounded-xl shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Kategori</label>
                            <input type="text" name="kategori" value="{{ old('kategori') }}" class="w-full border-gray-300 rounded-xl shadow-sm focus:border-indigo-500 focus:ring-indigo-500" placeholder="Opsional (mis. Guru, Satpam)">
                        </div>
                        <div class="md:col-span-2">
                            ...
                            <select name="bonus_scheme_id" class="w-full border-gray-300 rounded-xl shadow-sm focus:border-indigo-500 focus:ring-indigo-500">
                                ...
                                @foreach($bonusSchemes as $scheme)
                                    <option value="{{ $scheme->id }}" {{ old('bonus_scheme_id') == $scheme->id ? 'selected' : '' }}>{{ $scheme->nama }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                            ...
                            <div class="flex items-center gap-4 bg-gray-50 p-4 rounded-xl border border-gray-100">
                                ...
                                <div class="flex-1 flex gap-4 items-center">
                                    <div class="flex-1">
                                        ...
                                        <input type="time" name="details[{{ $h }}][jam_masuk]" value="{{ old('details.'.$h.'.jam_masuk') }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                                    </div>
                                    <span class="text-gray-400 mt-5">-</span>
                                    <div class="flex-1">
                                        <label class="block text-xs text-gray-500 mb-1">Jam Pulang</label>
                                        <input type="time" name="details[{{ $h }}][jam_pulang]" value="{{ old('details.'.$h.'.jam_pulang') }}" class="w-full border-gray-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                                    </div>
                                </div>
                            </div>
                        @endforeach
                ...
                <div class="flex justify-end gap-3">
                    ...
                    <button type="submit" class="px-5 py-2.5 bg-indigo-600 border border-transparent rounded-xl text-sm font-medium text-white hover:bg-indigo-700">Simpan Shift</button>
                </div>
            </form>
        </div>
    </div>
