    ...
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <h2 class="font-semibold text-2xl text-gray-800 leading-tight">
                {{ __('Edit Perangkat') }}: {{ $device->nama_mesin }}
            </h2>
        </div>
    </x-slot>
                ...
                <form action="{{ route('devices.update', $device->id) }}" method="POST" class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="nama_mesin" id="nama_mesin" value="{{ old('nama_mesin', $device->nama_mesin) }}" required class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="ip_address" id="ip_address" value="{{ old('ip_address', $device->ip_address) }}" required class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                        ...
                        <div>
                            ...
                            <input type="text" name="comm_key" id="comm_key" value="{{ old('comm_key', $device->comm_key) }}" class="w-full rounded-lg border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500">
                        </div>
                    </div>
                    
                    <!-- Status -->
                    <div class="mt-6 flex items-center">
                        <input type="checkbox" name="status" id="status" value="1" {{ old('status', $device->status) ? 'checked' : '' }} class="rounded border-gray-300 text-blue-600 shadow-sm focus:ring-blue-500">
